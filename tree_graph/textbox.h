//
// Created by gtstore on 03.02.2023.
//
#ifndef TEXTBOX_H_
#define TEXTBOX_H_
...
/* A utility function that can be used to create a tree graph rendering from a structure.
 *
 * Parameters:
...
textbox create_tree_graph(ParamType e,
                          std::size_t maxwidth,
                          ...
                          Separate1stParamTestFunc&& separate1st_test)
{
    ...
    auto param_range = count_children(e);
    if(param_range.first != param_range.second)
    {
        ...
        for(auto i = boxes.begin(); i != boxes.end(); ++i)
        {
            ...
            if(simple)
            {
                ...
                //result.hline(atom.size(), 3, 20, false,false);
            }
            else if(oneliner)
            {
